...
impl super::AlfenDriver {
    ...
    fn should_send_update(&self, effective: f32) -> (bool, bool) {
        ...
        (need_watchdog || need_change, need_change)
    }
    ...
    pub(crate) async fn poll_cycle(&mut self) -> Result<()> {
        ...
        if self.modbus_manager.is_some() {
            ...
            let (should_update, need_change) = self.should_send_update(effective);
            if should_update {
                if need_change {
