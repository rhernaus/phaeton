...
window.buildSection = function (container, key, sectionDef, cfg) {
  ...
  } else if (sectionDef.type === 'integer' || sectionDef.type === 'string') { const fieldEl = createInput(key, sectionDef, cfg, [key]); fieldEl.dataset.path = JSON.stringify([key]); body.appendChild(fieldEl); }
